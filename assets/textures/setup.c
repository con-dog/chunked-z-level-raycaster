...
extern World_Objects_Container *
setup_engine_textures(SDL_Renderer *renderer, char *root_manifest_file) {
  const char *manifest_json_string =
      read_asset_manifest_file(root_manifest_file);
  if (!manifest_json_string) {
    return NULL;
  }

  World_Objects_Container *world_objects_container =
      malloc(sizeof(World_Objects_Container));
  if (!world_objects_container) {
    free((void *)manifest_json_string);
    return NULL;
  }

  if (!parse_asset_manifest_json_string(world_objects_container,
...
bool parse_asset_manifest_json_string(
    World_Objects_Container *out_world_objects_container,
  ...
  if (!world_object_data ||
      !cJSON_IsArray(world_object_data)) // TODO ! Move to validation func
  {
    cJSON_Delete(root);
    return false;
  }
  ...
  out_world_objects_container->data =
      malloc(world_object_count * sizeof(World_Object *));
  ...
  cJSON_ArrayForEach(world_object, world_object_data) {
    ...
    out_world_objects_container->data[world_object_index] =
...
bool parse_frame_src_files(World_Object *world_object,
                           cJSON        *frame_src_files_array) {
...
bool parse_texture_fields(World_Object *world_object,
  ...
  // Parse src_directory
  cJSON *src_directory =
      cJSON_GetObjectItemCaseSensitive(json_object, "src_directory");
  ...
  cJSON *frame_src_files =
      cJSON_GetObjectItemCaseSensitive(json_object, "frame_src_files");
  if (!parse_frame_src_files(world_object, frame_src_files)) {
    return false;
  }

  cJSON *is_animated =
      cJSON_GetObjectItemCaseSensitive(json_object, "is_animated");
  if (!is_animated || !cJSON_IsBool(is_animated)) {
    return false;
  }
  world_object->animation_state.is_animated = cJSON_IsTrue(is_animated);

  cJSON *frame_duration =
      cJSON_GetObjectItemCaseSensitive(json_object, "frame_duration");
  if (!frame_duration || !cJSON_IsNumber(frame_duration)) {
    return false;
  }
  world_object->animation_state.frame_duration = frame_duration->valuedouble;

  cJSON *is_looping =
      cJSON_GetObjectItemCaseSensitive(json_object, "is_looping");
  if (!is_looping || !cJSON_IsBool(is_looping)) {
    return false;
  }
  world_object->animation_state.is_looping = cJSON_IsTrue(is_looping);
  ...
  cJSON *surface_type =
      cJSON_GetObjectItemCaseSensitive(json_object, "surface_type");
  cJSON *collision_mode =
      cJSON_GetObjectItemCaseSensitive(json_object, "collision_mode");
  ...
  // Parse dimensions
  cJSON *width =
      cJSON_GetObjectItemCaseSensitive(json_object, "expected_pixel_width");
  cJSON *height =
      cJSON_GetObjectItemCaseSensitive(json_object, "expected_pixel_height");
  ...
  // Parse scale mode
  cJSON *scale_mode =
      cJSON_GetObjectItemCaseSensitive(json_object, "use_scale_mode_nearest");
  ...
  world_object->animation_state.current_frame_index = 0;
  world_object->animation_state.frame_elapsed_time  = 0;
  world_object->animation_state.max_frame_index =
      world_object->textures.length - 1;

  return true;
}

bool process_world_objects(
  ...
  for (size_t i = 0; i < out_world_objects_container->length; i++) {
    for (size_t j = 0;
         j < out_world_objects_container->data[i]->frame_src_files.length;
         j++) {
      char temp_path[MAX_PATH_LENGTH];
      int  chars_written = snprintf(
          temp_path, MAX_PATH_LENGTH, "%s/%s",
      ...
      SDL_ScaleMode scale_mode =
          out_world_objects_container->data[i]->use_scale_mode_nearest
